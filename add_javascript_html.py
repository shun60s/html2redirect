...
#（内容）
# カレントディレクトリ以下の
# 既在のhtmlファイルの</head>の前にjava scriptのlocation.hrefを追加したhtmlファイルを生成する。
#
# (以下を設定する）
# 移転先のURL
url_org='http://example.jp/'
# 移動開始時間（mSec）例：5000は5秒を示す。
startup_time0='5000'
# <head>の次にcanonicalタグを追加したhtmlの出力ディレクトリー
dir_out='.\\output_html\\'
# ファイル拡張子
ext0='.html'
...
for path_org in List1:
    ...
    url1= "'" + url0 + "'"
    c1-=7  # </head>の手前の位置へ
    if c1 > 0 :   #  </head>　タグの位置が既知の場合は </head>の前に以下を追加する。
        # create java script sentence
        can0= '<SCRIPT type="text/javascript"> setTimeout(function(){location.href=' + url1 + '; }, ' + str(startup_time0) + '); </SCRIPT>'
        
        if c2a > -1 or c2b > -1:
            # <head> ～</head>の間にutf-8又はUTF-8 の記述がある場合。この判別方法は不完全である。
            all0 = data_org[0:c1] + can0.encode('utf-8') + data_org[c1:]
        else:
            # Windows 環境のデフォルト文字コード shift-JIS
            all0 = data_org[0:c1] + can0.encode('shift-jis') + data_org[c1:]
        #　上書きしてしまうので注意！
        fout= open(path0, 'wb')
        fout.write( all0 )
        fout.close()
    else:
        print ('ERROR: cannot find <head> tag. No proceed')
