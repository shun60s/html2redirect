...
#（内容）
# カレントディレクトリ以下の
# 既在のhtmlファイルの<head>の次にmeta refreshを追加したhtmlファイルを生成する。
#
# (以下を設定する）
# 移転先のURL
...
for path_org in List1:
    ...
    if c0 > 0 :   #  <head>　タグの位置が既知の場合は <head>の次に以下を追加する。
        ...
        can0= '<meta http-equiv="refresh" content="' + str(startup_time0) + ';URL=' + url0 + '">'
        
        if c2a > -1 or c2b > -1:
            # <head> ～</head>の間にutf-8又はUTF-8 の記述がある場合。この判別方法は不完全である。
            all0 = data_org[0:c0] + can0.encode('utf-8') + data_org[c0:]
        else:
            # Windows 環境のデフォルト文字コード shift-JIS
            all0 = data_org[0:c0] + can0.encode('shift-jis') + data_org[c0:]
        #　上書きしてしまうので注意！
        fout= open(path0, 'wb')
        fout.write( all0 )
        fout.close()
